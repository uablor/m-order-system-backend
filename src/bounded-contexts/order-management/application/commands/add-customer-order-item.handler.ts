...
export class AddCustomerOrderItemHandler implements ICommandHandler<AddCustomerOrderItemCommand> {
  ...
  async execute(command: AddCustomerOrderItemCommand): Promise<{ itemId: string }> {
    ...
    const allocatedQty = await this.coRepo.sumAllocatedQuantityForOrderItem(command.orderItemId);
    ...
    const unitCostLak = orderItem.quantity > 0 ? orderItem.finalCostLak / orderItem.quantity : 0;
    const item = CustomerOrderItemEntity.allocate({
